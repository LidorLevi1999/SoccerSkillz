...
using UnityEngine.Purchasing.Security;
...
#endif
	{
        ...
        /// string id
        /// bool modeTest
        public static event Action<string, bool> OnBuyPurchaseSuccessed = delegate { };
		/// <summary>
		/// 
		/// </summary>
		...
		public EStatusBuyIAP BuyProductByID(string productId)
		{
            ...
            {
                OnBuyPurchaseSuccessed?.Invoke(productId, true);
                return EStatusBuyIAP.Success;
            }
/*
            FirebaseManager.Instance.LogEvent(DMCF4AConst.EventNameBuyIAP, new Dictionary<string, object>
            {
                { "NameEvent", "BuyProductByID"},
                { "id", productId },
                { "country", F4ACoreManager.Instance.ipComponent.country },
                { "deviceUniqueIdentifier", F4ACoreManager.Instance.ipComponent.deviceUniqueIdentifier }
            });
*/
...
#region IStoreListener
        ...
        {
            OnBuyPurchaseSuccessed?.Invoke(args.purchasedProduct.definition.id, false);

            try
            {
                var pd = args.purchasedProduct;
            
               // FirebaseManager.Instance.LogEvent(DMCF4AConst.EventNameBuyIAP,
                new Dictionary<string, object>
                {
                    { "NameEvent", "ProcessPurchase"},
                    { "id",  pd.definition.id},
